...
# Configuration
app.config["DISABLE_ANALYTICS"] = (
    os.environ.get("DISABLE_ANALYTICS", "false").lower() == "true"
)
...
def _generate_all_caches_unified():
    ...
    def simple_extract_excerpt(content, max_words=50):
        ...
        content = re.sub(
            r'<label[^>]*class="margin-toggle sidenote-number"[^>]*></label><input[^>]*class="margin-toggle"[^>]*/>(<span class="sidenote">.*?</span>)',
            "",
            content,
            flags=re.DOTALL,
        )
        ...
        if full_path.parent.name == "essays":
                ...
                blog_posts.append(
                    {
                        ...
                        "unique_icon": generate_unique_svg_icon(
                            content_data["title"], size=24
                        ),
                    }
                )
            else:
                print(
                    f"DEBUG: Could not extract date from {full_path.name} in unified cache"
                )
                ...
                for quote in quotes:
                    ...
                    if clean_quote:
                        quotes_data[file_path].append(
                            {"text": clean_quote, "html": quote.strip()}
                        )
            ...
            for word in set(words):
                if len(word) > 3 and word not in [
                    ...
                    terms_data[word].append({"file": file_path, "context": word})
        ...
        for file_path, count in file_counts.items():
            ...
            if url:
                ...
                if title:  # Only include articles with valid titles
                    articles.append({"url": url, "title": title, "count": count})
    ...
    unified_cache = {
        ...
        "sidenotes": {"articles": dict(sidenotes_data), "total_count": total_sidenotes},
        "outlines": {
        ...
        "terms": {"terms": final_terms, "total_occurrences": total_term_occurrences},
    }
...
def generate_unique_svg_icon_OLD(title, size=24):
    ...
    if pattern_type == 0:  # Layered circles with gradients
        ...
        for i in range(3):
            ...
            opacity = 0.7 + i * 0.1
            shapes.append(
                f'<circle cx="{size//2}" cy="{size//2}" r="{radius}" fill="url(#{gradient_id})" opacity="{opacity}"/>'
            )
        ...
        # Center circle
        shapes.append(
            f'<circle cx="{center_x}" cy="{center_y}" r="{radius}" fill="none" stroke="url(#{gradient_id})" stroke-width="2" opacity="0.8"/>'
        )
        ...
        for i in range(6):
            ...
            shapes.append(
                f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{radius}" fill="none" stroke="url(#{gradient_id})" stroke-width="2" opacity="0.7"/>'
            )
        ...
        for i in range(12):
            ...
            shapes.append(
                f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{radius//2}" fill="none" stroke="{color2}" stroke-width="1" opacity="0.5"/>'
            )
        ...
        shapes.append(
            f'<polygon points="{" ".join(points)}" fill="none" stroke="url(#{gradient_id})" stroke-width="2.5" opacity="0.8"/>'
        )
        ...
        for i in range(6):
            ...
            shapes.append(
                f'<line x1="{center_x}" y1="{center_y}" x2="{x:.1f}" y2="{y:.1f}" stroke="{color2}" stroke-width="1.5" opacity="0.6"/>'
            )

        # Central sacred point
        shapes.append(
            f'<circle cx="{center_x}" cy="{center_y}" r="2" fill="none" stroke="{color1}" stroke-width="1.5" opacity="0.9"/>'
        )

    elif pattern_type == 3:  # Flowing wave interference - line art
        defs.append(
            f"""<linearGradient id="{gradient_id}" x1="0%" y1="50%" x2="100%" y2="50%">
            <stop offset="0%" stop-color="{color1}"/>
        ...
        for wave in range(3):
            ...
            shapes.append(
                f'<path d="{path_data}" stroke="url(#{gradient_id})" stroke-width="{stroke_width:.1f}" fill="none" opacity="{opacity:.2f}" stroke-linecap="round"/>'
            )

    elif pattern_type == 4:  # Sacred Golden Ratio Spiral - refined line art
        defs.append(
            f"""<linearGradient id="{gradient_id}" x1="20%" y1="20%" x2="80%" y2="80%">
        ...
        shapes.append(
            f'<path d="{path_data}" stroke="url(#{gradient_id})" stroke-width="3" fill="none" opacity="0.85" stroke-linecap="round"/>'
        )
        ...
        for i, fib in enumerate(fib_sizes):
            ...
            opacity = 0.4 - i * 0.08
            shapes.append(
                f'<rect x="{x}" y="{y}" width="{square_size}" height="{square_size}" fill="none" stroke="{color2}" stroke-width="1" opacity="{opacity:.2f}"/>'
            )

        # Sacred center - golden ratio point
        shapes.append(
            f'<circle cx="{center_x}" cy="{center_y}" r="1.5" fill="none" stroke="{color1}" stroke-width="2" opacity="0.9"/>'
        )

    elif pattern_type == 5:  # Tessellation pattern
        defs.append(
            f"""<pattern id="{gradient_id}" x="0" y="0" width="8" height="8" patternUnits="userSpaceOnUse">
        ...
        shapes.append(
            f'<polygon points="{" ".join(points)}" fill="url(#{gradient_id})" stroke="{color2}" stroke-width="1"/>'
        )
        ...
        def draw_branch(x, y, angle, length, depth):
            ...
            branches = [
                f'<line x1="{x:.1f}" y1="{y:.1f}" x2="{end_x:.1f}" y2="{end_y:.1f}" stroke="url(#{gradient_id})" stroke-width="{depth}" opacity="{0.8 if depth > 1 else 0.6}"/>'
            ]
            branches.extend(
        ...
        for x in range(spacing, size - spacing + 1, spacing):
            for y in range(spacing, size - spacing + 1, spacing):
                ...
                shapes.append(
                    f'<circle cx="{x}" cy="{y}" r="{dot_size}" fill="{color}" opacity="{opacity}"/>'
                )
        ...
        for i in range(3):
            for j in range(3):
                ...
                if (i + j) % 2 == 0:
                    shapes.append(
                        f'<polygon points="{x},{y} {x+tri_size},{y} {x+tri_size//2},{y+tri_size}" fill="url(#{gradient_id})" opacity="0.8"/>'
                    )
                else:
                    shapes.append(
                        f'<polygon points="{x},{y+tri_size} {x+tri_size},{y+tri_size} {x+tri_size//2},{y}" fill="{color2}" opacity="0.6"/>'
                    )

    elif pattern_type == 9:  # Organic bubbles
        defs.append(
            f"""<radialGradient id="{gradient_id}" cx="30%" cy="30%" r="70%">
            <stop offset="0%" stop-color="{color2}"/>
            <stop offset="100%" stop-color="{color1}"/>
        </radialGradient>"""
        )
        ...
        for i, (x, y, radius) in enumerate(bubble_positions):
            ...
            shapes.append(
                f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{radius}" fill="{bubble_color}" opacity="{opacity}"/>'
            )
        ...
        # Center circle
        shapes.append(
            f'<circle cx="{center_x}" cy="{center_y}" r="{radius//2}" fill="url(#{gradient_id})" opacity="0.9"/>'
        )
        ...
        for i in range(6):
            ...
            y = center_y + radius * math.sin(angle)
            shapes.append(
                f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{radius//3}" fill="none" stroke="url(#{gradient_id})" stroke-width="2" opacity="0.8"/>'
            )
        ...
        for i in range(6):
            ...
            shapes.append(
                f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{radius//4}" fill="none" stroke="{color2}" stroke-width="1" opacity="0.6"/>'
            )
        ...
        for i in range(6):
            ...
            shapes.append(
                f'<line x1="{x1:.1f}" y1="{y1:.1f}" x2="{x2:.1f}" y2="{y2:.1f}" stroke="{color2}" stroke-width="1" opacity="0.4"/>'
            )
        ...
        for i in range(num_petals):
            ...
            shapes.append(
                f'<ellipse cx="{x:.1f}" cy="{y:.1f}" rx="{size//8}" ry="{size//6}" fill="url(#{gradient_id})" opacity="0.8" transform="rotate({i * 360 / num_petals} {x:.1f} {y:.1f})"/>'
            )

        # Center
        shapes.append(
            f'<circle cx="{size//2}" cy="{size//2}" r="{size//10}" fill="{color1}"/>'
        )
        ...
        for x in range(diamond_size, size, diamond_size * 2):
            for y in range(diamond_size, size, diamond_size * 2):
                ...
                shapes.append(
                    f'<polygon points="{" ".join(points)}" fill="{color}" opacity="0.7"/>'
                )
        ...
        for wave in range(5):
            ...
            for x in range(0, size, 2):
                ...
                path_data += f" L {x},{y:.1f}"
            shapes.append(
                f'<path d="{path_data}" stroke="url(#{gradient_id})" stroke-width="2" fill="none" opacity="{0.9 - wave * 0.15}"/>'
            )
        ...
        for row in range(4):
            for col in range(4):
                ...
                if x < size and y < size:
                    ...
                    shapes.append(
                        f'<polygon points="{" ".join(points)}" fill="{color}" opacity="0.6" stroke="{color2}" stroke-width="1"/>'
                    )
        ...
        for i in range(4):
            ...
            opacity = 0.7 - i * 0.1
            shapes.append(
                f'<polygon points="{x1:.1f},{y1:.1f} {x2:.1f},{y2:.1f} {x3:.1f},{y3:.1f}" fill="none" stroke="url(#{gradient_id})" stroke-width="2" opacity="{opacity}"/>'
            )
        ...
        for i in range(5):
            ...
            opacity = 0.6 - i * 0.08
            shapes.append(
                f'<polygon points="{x1:.1f},{y1:.1f} {x2:.1f},{y2:.1f} {x3:.1f},{y3:.1f}" fill="none" stroke="{color2}" stroke-width="1" opacity="{opacity}"/>'
            )

        # Central bindu (divine point)
        shapes.append(
            f'<circle cx="{center_x}" cy="{center_y}" r="{size//20}" fill="{color1}" opacity="0.9"/>'
        )

        # Outer protective circles
        shapes.append(
            f'<circle cx="{center_x}" cy="{center_y}" r="{outer_radius * 1.1}" fill="none" stroke="{color2}" stroke-width="1" opacity="0.5"/>'
        )
        shapes.append(
            f'<circle cx="{center_x}" cy="{center_y}" r="{outer_radius * 1.25}" fill="none" stroke="{color1}" stroke-width="1" opacity="0.3"/>'
        )
        ...
        for x in range(0, size, tile_size):
            for y in range(0, size, tile_size):
                ...
                if tile_hash == 0:
                    shapes.append(
                        f'<rect x="{x}" y="{y}" width="{tile_size}" height="{tile_size}" fill="{color1}" opacity="0.8"/>'
                    )
                elif tile_hash == 1:
                    shapes.append(
                        f'<circle cx="{x + tile_size//2}" cy="{y + tile_size//2}" r="{tile_size//3}" fill="{color2}" opacity="0.7"/>'
                    )
                elif tile_hash == 2:
                    points = f"{x},{y+tile_size} {x+tile_size//2},{y} {x+tile_size},{y+tile_size}"
                    shapes.append(
                        f'<polygon points="{points}" fill="{color1}" opacity="0.6"/>'
                    )
                else:
                    shapes.append(
                        f'<rect x="{x}" y="{y}" width="{tile_size}" height="{tile_size}" fill="{color2}" opacity="0.5" rx="{tile_size//4}"/>'
                    )
        ...
        for i in range(4):
            ...
            rotation = i * 45
            shapes.append(
                f'<circle cx="{size//2}" cy="{size//2}" r="{radius}" fill="none" stroke="url(#{gradient_id})" stroke-width="2" opacity="{0.8 - i*0.15}" transform="rotate({rotation} {size//2} {size//2})"/>'
            )
            # Small planet
            planet_x = size // 2 + radius
            planet_y = size // 2
            shapes.append(
                f'<circle cx="{planet_x}" cy="{planet_y}" r="3" fill="{color2}" transform="rotate({rotation} {size//2} {size//2})"/>'
            )

    elif pattern_type == 18:  # Woven pattern
        defs.append(
            f"""<pattern id="{gradient_id}" x="0" y="0" width="6" height="6" patternUnits="userSpaceOnUse">
        ...
        for i in range(6):
            x = i * size // 6
            shapes.append(
                f'<rect x="{x}" y="0" width="{size//12}" height="{size}" fill="url(#{gradient_id})" opacity="0.7"/>'
            )
            shapes.append(
                f'<rect x="0" y="{x}" width="{size}" height="{size//12}" fill="{color2}" opacity="0.5"/>'
            )
        ...
        shapes.append(
            f'<polygon points="{x1:.1f},{y1:.1f} {x2:.1f},{y2:.1f} {x3:.1f},{y3:.1f}" fill="none" stroke="url(#{gradient_id})" stroke-width="3" opacity="0.9"/>'
        )
        ...
        shapes.append(
            f'<polygon points="{center_x:.1f},{y1_inv:.1f} {x2_inv:.1f},{y2_inv:.1f} {x3_inv:.1f},{y3_inv:.1f}" fill="none" stroke="{color2}" stroke-width="2" opacity="0.8"/>'
        )
        ...
        for i in range(3):
            ...
            opacity = 0.7 - i * 0.15
            shapes.append(
                f'<polygon points="{x1_i:.1f},{y1_i:.1f} {x2_i:.1f},{y2_i:.1f} {x3_i:.1f},{y3_i:.1f}" fill="none" stroke="{color1}" stroke-width="1" opacity="{opacity}"/>'
            )

        # Central point of unity
        shapes.append(
            f'<circle cx="{center_x}" cy="{center_y}" r="{size//25}" fill="{color1}" opacity="1.0"/>'
        )

        # Corner vertices (tetraktys dots)
        vertex_radius = size // 30
        shapes.append(
            f'<circle cx="{x1:.1f}" cy="{y1:.1f}" r="{vertex_radius}" fill="{color2}" opacity="0.8"/>'
        )
        shapes.append(
            f'<circle cx="{x2:.1f}" cy="{y2:.1f}" r="{vertex_radius}" fill="{color2}" opacity="0.8"/>'
        )
        shapes.append(
            f'<circle cx="{x3:.1f}" cy="{y3:.1f}" r="{vertex_radius}" fill="{color2}" opacity="0.8"/>'
        )
...
def _convert_unified_outlines_cache(unified_cache):
    ...
    articles_list.sort(
        key=lambda x: x["date"] if x["date"] else datetime(1900, 1, 1), reverse=True
    )
    return {
        "articles": articles_list,
        "total_count": unified_cache.get(
            "total_count", sum(len(article["outlines"]) for article in articles_list)
        ),
    }
...
def _convert_unified_sidenotes_cache(unified_cache):
    ...
    print(
        f"Sidenotes conversion: {len(articles_list)} articles processed from {len(unified_cache.get('articles', {}))} files"
    )
    return {
        "articles": articles_list,
        "total_count": unified_cache.get(
...
def _extract_all_sidenotes_cached():
            ...
            if file_sidenotes:
                ...
                if full_matches:
                    ...
                    for i, sidenote in enumerate(file_sidenotes):
                        ...
                        # Clean up the sidenote text (remove extra whitespace)
                        sidenote_text = re.sub(r"\s+", " ", sidenote_text).strip()
                        cleaned_sidenotes.append({"text": sidenote_text, "id": None})
    ...
    # Update cache
    result = {"articles": articles_list, "total_count": total_count}
        ...
        try:
            ...
            if headings:
                ...
                for level, heading_id, heading_text in headings:
                    ...
                    if (
                        level == "1"
                        and heading_text.strip() == content_data["title"].strip()
                    ):
                        continue
                    ...
                    cleaned_headings.append(
                        {
                            ...
                            "anchor_url": (
                                f"{url_path}#{heading_id}" if heading_id else url_path
                            ),
                        }
                    )
...
def _extract_all_connections_cached():
        ...
        try:
            ...
            for link_url, link_text in links:
                if (
                    ...
                    and not link_url.startswith("/static")
                    ...
                    articles_data[source_url]["outgoing_connections"].append(
                        {"target_url": link_url, "link_text": link_text}
                    )
    ...
    total_outgoing = sum(
        len(article["outgoing_connections"]) for article in articles_list
    )
    total_incoming = sum(
        len(article["incoming_connections"]) for article in articles_list
    )
...
def graph_data():
    ...
    for article in connections_data["articles"]:
        ...
        connections_list = article.get(
            "outgoing_connections", article.get("connections", [])
        )
...
def archive_year(year):
    ...
    grouped_posts = dict(
        sorted(grouped_posts.items(), key=lambda x: month_order[x[0]], reverse=True)
    )
...
def directory_index():
    ...
    is_image_gallery = (
        len(image_items) >= 3
        and len(total_files) > 0
        and (len(image_items) / len(total_files)) >= 0.5
    )
...
def serve_path(path):
    ...
    if full_path.is_dir():
        ...
        if full_path.parent != DATA_DIR:  # Don't show parent for root-level content
            ...
            parent_display_name = (
                parent_path.name.replace("-", " ").replace("_", " ").title()
            )
        ...
        prev_post = None
        next_post = None
        if "essays" in path or (
            "artificial-intelligence" in path and "writings" in path
        ):
            related_posts = find_related_posts(str(full_path.relative_to(DATA_DIR)))
            prev_post, next_post = find_adjacent_posts(
                str(full_path.relative_to(DATA_DIR))
            )
        ...
        if content_text:
            ...
            description = (
                first_para[:200] + "..." if len(first_para) > 200 else first_para
            )
...
def _get_article_icon_cached(article_path):
    ...
    for post in posts:
        if post["url"] == article_path:
            if "unique_icon" in post and post["unique_icon"]:
                ...
                return {"success": True, "icon": icon_svg, "title": post["title"]}
    ...
    try:
        ...
        if article_path.endswith("/") or file_path.is_dir():
            ...
            index_path = (
                file_path / "index.md"
                if file_path.is_dir()
                else file_path.with_name("index.md")
            )
    ...
    title_guess = (
        article_path.split("/")[-1].replace("-", " ").replace("_", " ").title()
    )
    if title_guess:
...
def api_search():
    ...
    def search_path(current_path: Path, display_path: str = ""):
        ...
        for item in current_path.iterdir():
            ...
            item_display_path = (
                f"{display_path}/{item.name}" if display_path else item.name
            )
            ...
            if query in node_name or query in node_path or query in node_content:
                ...
                result = {
                    ...
                    "type": (
                        "directory"
                        if item.is_dir()
                        else ("article" if item.suffix == ".md" else "file")
                    ),
                    "path": relative_path,
...
def collect_blog_posts():
    ...
    blog_dirs = [
        DATA_DIR / "essays",
        DATA_DIR
        / "artificial-intelligence",  # This will pick up root AI posts and scan subdirs
    ]
                ...
                try:
                    ...
                    try:
                        ...
                        for line in lines:
                            ...
                            clean_line = re.sub(
                                r"[*_]{1,3}([^*_]+)[*_]{1,3}", r"\1", clean_line
                            )  # Remove bold/italic
                            clean_line = re.sub(
                                r"`([^`]+)`", r"\1", clean_line
                            ...
                            if (
                                clean_line and len(clean_line) > 20
                    ...
                    posts.append(
                        {
                            ...
                            "pub_date": pub_date,
                            "category": category
                            or item.parent.name.replace("-", " ").title(),
                            "content": (
...
class MetadataCache:
                ...
                articles.append(
                    {
                        ...
                        "sidenotes": [
                            {"text": s["text"], "id": s.get("id")} for s in sidenotes
                        ],
    ...
    def get_connections(self):
        ...
        for file_path, outgoing_list in outgoing_refs.items():
            ...
            for conn in outgoing_list:
                processed_outgoing.append(
                    {"target_url": conn["url"], "link_text": conn["text"]}
                )
        ...
        # Calculate totals
        total_outgoing = sum(
            len(article["outgoing_connections"]) for article in articles
        )
        total_incoming = sum(
            len(article["incoming_connections"]) for article in articles
        )
    ...
    if date_match:
        try:
            ...
            pub_date = datetime.strptime(
                date_match.group(1) + f"-{day:02d}", "%Y-%m-%d"
            )
            return pub_date
        except:
            pass
        ...
        try:
            ...
            try:
                ...
                month_match = re.search(
                    r"\*([A-Za-z]+)\s+" + str(year) + r"\*", first_few_lines
                )
                if month_match:
    ...
    if content_data and content_data["metadata"].get("date"):
        try:
            if isinstance(content_data["metadata"]["date"], list):
                pub_date = datetime.strptime(
                    content_data["metadata"]["date"][0], "%Y-%m-%d"
                )
            else:
                pub_date = datetime.strptime(
                    str(content_data["metadata"]["date"]), "%Y-%m-%d"
                )
            return pub_date
        except:
            pass
...
def _collect_all_blog_posts_cached():
    ...
    if (
        _blog_posts_cache["data"] is not None
        and current_time - _blog_posts_cache["timestamp"] < CACHE_TTL
        and _blog_posts_cache["timestamp"] > _force_cache_clear
    ):
...
def preload_sidenotes():
    ...
    load_time = time.time() - start_time
    print(
        f"Extracted {sidenotes_data['total_count']} sidenotes from {len(sidenotes_data['articles'])} articles in {load_time:.2f}s"
    )
...
def preload_outlines():
    ...
    load_time = time.time() - start_time
    print(
        f"Extracted {outlines_data['total_count']} headings from {len(outlines_data['articles'])} articles in {load_time:.2f}s"
    )
...
def preload_quotes():
    ...
    load_time = time.time() - start_time
    print(
        f"Extracted {quotes_data['total_count']} quotes from {len(quotes_data['articles'])} articles in {load_time:.2f}s"
    )
...
def preload_connections():
    ...
    load_time = time.time() - start_time
    print(
        f"Extracted {connections_data['total_count']} cross-references in {load_time:.2f}s"
    )
...
def _extract_all_external_links_cached():
    ...
    if (
        _external_links_cache["data"] is not None
        and current_time - _external_links_cache["timestamp"] < CACHE_TTL
        and _external_links_cache["timestamp"] > _force_cache_clear
    ):
    ...
    for post in posts:
        ...
        matches = re.findall(
            external_link_pattern, post["content"], re.IGNORECASE | re.DOTALL
        )
...
def preload_external_links():
    ...
    load_time = time.time() - start_time
    print(
        f"Extracted {links_data['total_count']} external links from {len(links_data['articles'])} articles in {load_time:.2f}s"
    )
...
def _extract_all_terms_cached():
    ...
    posts = _collect_all_blog_posts_cached()
    term_occurrences = defaultdict(
        list
    )  # term -> [(article_title, article_url, count)]
    ...
    for post in posts:
        ...
        capitalized_terms = re.findall(
            r"\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b", clean_content
        )
        ...
        for term, count in terms_in_post.items():
            if count >= 1:  # Must appear at least once
                term_occurrences[term].append(
                    {"title": post["title"], "url": post["url"], "count": count}
                )
    ...
    result = {
        ...
        "total_occurrences": sum(
            term_data["total_count"] for term_data in sorted_terms.values()
        ),
    }
...
def preload_terms():
    ...
    load_time = time.time() - start_time
    print(
        f"Extracted {terms_data['total_terms']} terms with {terms_data['total_occurrences']} total occurrences in {load_time:.2f}s"
    )
...
def find_related_posts(current_post_path, limit=3):
    ...
    current_post_url = (
        "/" + current_post_path[:-3]
        if current_post_path.endswith(".md")
        else "/" + current_post_path
    )
    ...
    for post in posts:
        ...
        current_desc_words = (
            set(current_post["description"].lower().split())
            if current_post["description"]
            else set()
        )
        post_desc_words = (
            set(post["description"].lower().split()) if post["description"] else set()
        )
...
def sitemap():
    ...
    # Group by type
    grouped_sitemap = {"homepage": [], "directory": [], "article": [], "sitemap": []}
...
def sitemap_xml():
    ...
    for item in sitemap_data:
        ...
        if "modified" in item:
            xml_content += (
                f'    <lastmod>{item["modified"].strftime("%Y-%m-%d")}</lastmod>\n'
            )
        xml_content += "  </url>\n"
...
def rss_feed():
    ...
    rss_content += (
        "    <managingEditor>me@kennethreitz.org (Kenneth Reitz)</managingEditor>\n"
    )
    ...
    for post in posts:
        ...
        try:
            ...
            full_content = (
                f"{post['description']} <!-- Error loading full content: {str(e)} -->"
            )
