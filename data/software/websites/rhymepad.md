...
Every word is mapped to its phonemes using the CMU Pronouncing Dictionary, with lyric-friendly fallbacks for the way people actually write — `runnin'` becomes `running`, possessives resolve, and made-up words get a spelling heuristic so the page doesn't go silent when you invent a word. Perfect rhymes share everything from the last stressed vowel on. Slant rhymes share just the vowels. Same sound, same color.

## What It Does

- **Phonetic rhyme detection as you type** — end, internal, and slant rhymes, color-coded by sound.
- **Rhyme and synonym lookup** — frequency-ranked rhymes, near rhymes by syllable count, and WordNet synonyms by part of speech.
- **Draggable stanza reordering** — move the shape of a poem around without retyping it.
- **Synthesized beats** with adjustable tempo, for when the words want something to lean on.
