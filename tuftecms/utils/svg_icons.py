...
def generate_unique_svg_icon(title, size=24):
    ...
    gradient_def += (
        "</linearGradient>" if gradient_type == "linear" else "</radialGradient>"
    )
    ...
    if rng.random() > 0.5:
        gradient_def2 = (
            f'<linearGradient id="{gradient_id2}" x1="0%" y1="0%" x2="100%" y2="100%">'
        )
        for i, color in enumerate(reversed(colors)):
            offset = (i * 100) // (len(colors) - 1) if len(colors) > 1 else 50
            gradient_def2 += (
                f'<stop offset="{offset}%" stop-color="{color}" stop-opacity="0.8"/>'
            )
        gradient_def2 += "</linearGradient>"
        defs.append(gradient_def2)

    # Choose a composition style for better aesthetics
    composition_style = rng.choice(
        ["centered", "spiral", "grid", "organic", "geometric"]
    )
    ...
    if composition_style == "centered":
        ...
        for i in range(num_shapes):
            ...
            if shape_type == "circle":
                r = size * 0.4 * scale
                shapes.append(
                    f'<circle cx="{size//2}" cy="{size//2}" r="{r:.1f}" fill="url(#{gradient_id})" opacity="{opacity:.2f}"/>'
                )
                ...
                x = (size - s) / 2
                shapes.append(
                    f'<rect x="{x:.1f}" y="{x:.1f}" width="{s:.1f}" height="{s:.1f}" rx="{s*0.2:.1f}" fill="url(#{gradient_id})" opacity="{opacity:.2f}" transform="rotate({i*15} {size//2} {size//2})"/>'
                )

            elif shape_type == "star":
                points = create_star_points(
                    size // 2, size // 2, size * 0.4 * scale, size * 0.2 * scale, 5 + i
                )
                shapes.append(
                    f'<polygon points="{points}" fill="url(#{gradient_id})" opacity="{opacity:.2f}"/>'
                )
            ...
            for j in range(grid_size):
                if rng.random() > 0.3:  # 70% chance to place element
                    ...
                    fill = (
                        f"url(#{gradient_id})"
                        if (i + j) % 2 == 0
                        else colors[(i + j) % len(colors)]
                    )

                    if shape_type == "circle":
                        shapes.append(
                            f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{s/2:.1f}" fill="{fill}" opacity="{opacity:.2f}"/>'
                        )
                    else:
                        shapes.append(
                            f'<rect x="{x-s/2:.1f}" y="{y-s/2:.1f}" width="{s:.1f}" height="{s:.1f}" rx="{s*0.2:.1f}" fill="{fill}" opacity="{opacity:.2f}"/>'
                        )
        ...
        for i in range(num_blobs):
            ...
            shapes.append(
                f'<path d="{path_data}" fill="{fill}" opacity="{opacity:.2f}"/>'
            )
        ...
        if rng.random() > 0.5:
            ...
            for i in range(3):
                ...
                shapes.append(
                    f'<polygon points="{x1:.1f},{y1:.1f} {x2:.1f},{y2:.1f} {x3:.1f},{y3:.1f}" fill="{fill}" opacity="{opacity:.2f}" transform="rotate({rotation} {cx} {cy})"/>'
                )
        else:
            ...
            for i in range(4):
                ...
                shapes.append(
                    f'<polygon points="{diamond}" fill="{fill}" opacity="{opacity:.2f}"/>'
                )
    ...
    if rng.random() > 0.6:
        ...
        for _ in range(num_accents):
            ...
            shapes.append(
                f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{r:.1f}" fill="{colors[-1]}" opacity="{opacity:.2f}"/>'
            )
...
def create_blob_path(cx, cy, radius, rng):
    ...
    for i in range(num_points):
        ...
        path_data += (
            f" Q {cp1x:.1f},{cp1y:.1f} {points[next_i][0]:.1f},{points[next_i][1]:.1f}"
        )
